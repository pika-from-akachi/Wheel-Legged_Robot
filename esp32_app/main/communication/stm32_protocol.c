                                      ...
                                      uint16_t *out_len)
{
    ...
    const size_t needed = STM32_PKT_HEADER_LEN + payload_len + STM32_PKT_CRC_LEN;
    if (needed > out_capacity || payload_len > STM32_MAX_PAYLOAD) {
        return ESP_ERR_NO_MEM;
    }

    uint16_t idx = 0;
    out[idx++] = STM32_PKT_START1;
    out[idx++] = STM32_PKT_START2;
    out[idx++] = type;
    out[idx++] = payload_len;
    if (payload_len > 0) {
    ...
    const uint16_t crc = stm32_protocol_crc16(&out[2], payload_len + 2);
...
{
    return offset + STM32_PKT_HEADER_LEN + payload_len + STM32_PKT_CRC_LEN <= buffer_len;
}
