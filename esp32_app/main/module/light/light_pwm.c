...
static light_pwm_config_t s_config;
static light_pwm_status_t s_status;
...
static esp_err_t write_duty(uint8_t percent)
{
...
esp_err_t light_pwm_init(const light_pwm_config_t *config)
{
    ...
    s_status.mode = config->initial_duty_percent > 0 ? LIGHT_PWM_MODE_ON : LIGHT_PWM_MODE_OFF;
...
{
    if (channel != 0) {
        s_status.last_error = "invalid_channel";
        return ESP_ERR_INVALID_ARG;
    }
    if (percent > 100) {
    ...
    const esp_err_t ret = write_duty(percent);
    if (ret != ESP_OK) {
    ...
    s_status.mode = percent > 0 ? LIGHT_PWM_MODE_ON : LIGHT_PWM_MODE_OFF;
    s_status.last_error = NULL;
    return ESP_OK;
}
...
esp_err_t light_pwm_set_mode(light_pwm_mode_t mode)
{
    ...
    uint8_t duty = s_status.duty_percent;
    switch (mode) {
    case LIGHT_PWM_MODE_OFF:
        duty = 0;
        break;
    case LIGHT_PWM_MODE_ON:
        duty = duty == 0 ? 100 : duty;
        break;
    case LIGHT_PWM_MODE_BREATH:
    case LIGHT_PWM_MODE_FLASH:
        s_status.mode = mode;
        s_status.last_error = NULL;
        return ESP_OK;
    default:
        s_status.last_error = "invalid_mode";
        return ESP_ERR_INVALID_ARG;
    }

    const esp_err_t ret = write_duty(duty);
    if (ret != ESP_OK) {
        s_status.last_error = "ledc_write_failed";
        return ret;
    }
    s_status.duty_percent = duty;
    s_status.mode = mode;
    s_status.last_error = NULL;
    return ESP_OK;
}
